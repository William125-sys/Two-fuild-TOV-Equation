...
# ========== 主程序 ==========
def main():
    print("解决TOV方程的耦合系统 - 核物质和暗物质 (使用MeV/fm^3单位)")

    try:
        eos1 = np.loadtxt('eos1.txt')
        eos2 = np.loadtxt('0.1 300.txt')
    except FileNotFoundError as e:
        print(f"错误：找不到EOS数据文件: {e}")
        return

    if eos1.size == 0 or eos2.size == 0:
        raise ValueError('EOS数据加载失败，请检查文件路径和内容')

    print("\n请输入参数：")
    rho1_min = float(input('请输入核物质中心能量密度的最小值(MeV/fm^3): '))
    rho1_max = float(input('请输入核物质中心能量密度的最大值(MeV/fm^3): '))
    rho1_step = float(input('请输入核物质中心能量密度的步长(0表示保持为最小值): '))

    rho2_min = float(input('请输入暗物质中心能量密度的最小值(MeV/fm^3): '))
    rho2_max = float(input('请输入暗物质中心能量密度的最大值(MeV/fm^3): '))
    rho2_step = float(input('请输入暗物质中心能量密度的步长(0表示保持为最小值): '))
    ...
    for rho1_center in rho1_values:
        for rho2_center in rho2_values:
            print(
                f'正在计算 rho1={rho1_center / conversion_factor:.2e} MeV/fm^3, rho2={rho2_center / conversion_factor:.2e} MeV/fm^3...')
            ...
            if p1_center is None or p2_center is None:
                warnings.warn(f'无法为rho1={rho1_center:.2e}或rho2={rho2_center:.2e}找到初始压力，跳过')
                continue
            ...
            try:
                ...
                print(
                    f'结果: M1={M1_final_Msun:.3f} M☉, M2={M2_final_Msun:.3f} M☉, R1={R1_km:.2f} km, R2={R2_km:.2f} km')

            except Exception as e:
                warnings.warn(f'计算rho1={rho1_center:.2e}, rho2={rho2_center:.2e}时出错: {e}')

    if not results:
        raise ValueError('没有生成任何结果，请检查输入参数和EOS数据')

    results_array = np.array(results)

    print(
        '\n结果表格: [rho1_center (MeV/fm^3), rho2_center (MeV/fm^3), M1_final (M☉), M2_final (M☉), R1 (km), R2 (km)]')
    print(results_array)
    ...
    try:
        ...
        print('结果已成功保存到 TOV_results.txt')
    except Exception as e:
        print(f'无法写入文件，请在命令行窗口查看结果: {e}')
        print(results_array)
