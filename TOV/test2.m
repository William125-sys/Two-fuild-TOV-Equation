...
%此版本为监控剖面图，并且提供了压强的变化，监控仍然需要改进，需要让数据读进文件
%需要改步长
clear all;
close all;
clc;
...
eos2 = load('1e-11 300.txt'); % 暗物质的EOS [p(MeV/fm^3), rho(MeV/fm^3)]
...
for rho1_center = rho1_values
    for rho2_center = rho2_values
        ...
        % 调用求解器
        try
            [M1_final, M2_final, R1, R2,mon] = solve_TOV(p1_center, p2_center, eos1, eos2, G,c);
...
% ========== TOV方程求解函数 ==========
function [M1_final, M2_final, R1, R2,monitor_data] = solve_TOV(p1_center, p2_center, eos1, eos2, G, c)
    ...
    monitor_data = zeros(7000, 7);
    monitor_data(1, :) = [1, r, M1, M2, 0, 0,0]; 
    ...
    while  (~p1_zero || ~p2_zero)%当至少一个p1_zero或p2_zero为false时，循环继续；若两个均为true时，循环停止
        ...
        disp(p2);
        disp(step)
        if p2 <1e-100 && p1 < 1e-100 %当暗物质压强小于10e-30Pa,退出循环
            break;
        end
        step = step + 1;
        ...
        %if p1_zero && p2_zero
            %break;
        %end
        monitor_data(step+1, :) = [step+1, r, M1, M2, p1, p2,rho2];
    end

    monitor_data = monitor_data(1:step+1, :);
